...
export function Canvas({ previewWidth = 'desktop', zoom = 100, previewValues }: CanvasProps = {}) {
  ...
  return (
    ...
    >
      {/* Disable link navigation inside the canvas — clicks bubble to EditableBlock for select/drag.
          Scope to <a> tags only; <button>s would catch our editor toolbar buttons.
          Mobile preview: apply the same column-stacking rule the email uses at <600px. */}
      <style>{`
        .loomi-v2-canvas a { pointer-events: none !important; cursor: inherit !important; }
        .loomi-v2-canvas-mobile .loomi-mobile-stack {
...
function RenderedBlock({ block, settings }: { block: Block; settings: EmailTemplate['settings'] }) {
