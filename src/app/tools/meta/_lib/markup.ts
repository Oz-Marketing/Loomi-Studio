...
/**
 * Normalize a possibly-missing markup factor to a safe number. §0.1: an
 * unconfigured markup surfaces as 0 (→ a visibly-broken $0 target) rather than
 * a plausible-but-wrong default. The client never holds a markup literal; this
 * just guards the server-sent value before it's used in display math.
 */
export function effMarkupOf(markup: number | null | undefined): number {
  return typeof markup === 'number' && Number.isFinite(markup) && markup > 0 ? markup : 0;
}
